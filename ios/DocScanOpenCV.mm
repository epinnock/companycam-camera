...
-(BOOL)setPreviewBytes:(UIImage *)image {
    ...
    } else if (currentUNIXTimeMS - self.lastScanMS > 500.0) {
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.debugImage != nil) {
        ...
        float scaleW = (float)self.widthOverlay/(float)self.widthOrig;
        float scaleH = (float)self.heightOverlay/(float)self.heightOrig;
        float scale = 0.6f * MIN(scaleW, scaleH);
        CGContextDrawImage(
           context,
           CGRectMake(0, 0, scale*self.widthOrig, scale*self.heightOrig),
