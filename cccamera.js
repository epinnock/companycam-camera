...
class CCCamera extends React.Component {
  ...
  _persistCameraModes = async () => {
    try {
      const persistedModes = await AsyncStorage.multiGet([
        PERSIST_FASTCAM_MODE,
        PERSIST_FLASH_MODE,
        PERSIST_RESOLUTION_MODE,
      ]);
      ...
      for (const modeKeyValuePair of persistedModes) {
        ...
        switch (persistMode) {
          case PERSIST_FLASH_MODE:
            nextState.flashMode = value
              ? parseInt(value, 10)
              : constants.FlashMode.off;
            break;
          case PERSIST_FASTCAM_MODE:
            nextState.cameraMode = value
              ? parseInt(value, 10)
              : constants.CameraMode.photo;
            break;
          case PERSIST_RESOLUTION_MODE:
            nextState.resolutionMode = value
              ? parseInt(value, 10)
              : constants.ResolutionMode.normal;
            break;
          default:
            break;
        }
